...
def run(args):
    print('| Step: make_ins_seg_label |')

    if not os.path.isdir(args.ins_seg_save_dir):
        os.makedirs(args.ins_seg_save_dir)

    print('Loading dataset')
    dataset = MakeIRLabelImageFolder(args)
    dataset = [Subset(dataset, np.arange(i, len(dataset), args.num_gpus))
    ...
    model = EdgeDisplacement(backbone)
    ...
    model.eval()
    multiprocessing.spawn(_work, nprocs=args.num_gpus, args=(model, dataset, args), join=True)


def _work(process_id, model, dataset, args):
    data_loader = DataLoader(dataset[process_id], shuffle=False,
        num_workers=args.num_threads // args.num_gpus, pin_memory=False)
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for i, (image, cam, target, name) in enumerate(dataloader):
            data_time.update(time.time() - end)

            size = np.asarray(args.image_size)

            edge, dp = model(image.cuda(non_blocking=True))

            dp = dp.cpu().numpy()

            cam_dict = np.load(args.cam_out_dir + '/' + name + '.npy', allow_pickle=True).item()
            ...
            np.save(os.path.join(args.ins_seg_save_dir, name + '.npy'), detected)
